    ...
    >
      ...
      {onRemove && (
        <button
          ...
          className="ml-0.5 hover:text-gray-700 transition-colors"
        >×</button>
      )}
    </span>
  )
...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      {icon && <div className="text-gray-400 mb-4 opacity-40">{icon}</div>}
      <p className="text-md font-medium text-gray-500">{title}</p>
      {description && <p className="text-sm text-gray-400 mt-1.5 max-w-xs">{description}</p>}
