  ...
  return (
    <div className="page-container">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-medium text-ink">归档卡片</h1>
        <span className="text-sm text-ink-muted">({cards.length})</span>
      </div>
          ...
          {cards.map(card => (
            <div key={card.id} className="card-base px-4 py-3.5 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-md text-ink truncate">{card.title}</p>
                <p className="text-xs text-ink-muted mt-0.5">已学 {card.study_count} 次</p>
              </div>
